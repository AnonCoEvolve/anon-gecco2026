...
"""Home of the `Sequentialga` model.
"""
...
SINGLE_LAYER_OUTPUT_ERROR_MSG = ('All layers in a Sequentialga model should have '
                                 'a single output tensor. For multi-output '
                                 'layers, use the functional API.')


@keras_export('keras.Sequential', 'keras.models.Sequential')
class Sequentialga(training.Model):
  """`Sequentialga` groups a linear stack of layers into a `tf.keras.Model`.

  `Sequentialga` provides training and inference features on this model.

  Examples:

  >>> # Optionally, the first layer can receive an `input_shape` argument:
  >>> model = tf.keras.Sequentialga()
  ...
  def __init__(self, layers=None, name=None):
    """Creates a `Sequentialga` model instance.

    Args:
      layers: Optional list of layers to add to the model.
      name: Optional name for the model.
    """
    super(Sequentialga, self).__init__(name=name, autocast=False)
  ...
  @property
  def layers(self):
    # Historically, `Sequentialga.layers` only returns layers that were added
    # via `add`, and omits the auto-generated `InputLayer` that comes at the
    # bottom of the stack.
    # `Trackable` manages the `_layers` attributes and does filtering
    # over it.
    layers = super(Sequentialga, self).layers
  ...
  def add(self, layer):
    ...
    Raises:
        ...
        ValueError: In case the `layer` argument has
            ...
            somewhere else (forbidden in `Sequentialga` models).
    """
  ...
  def build(self, input_shape=None):
    if self._is_graph_network:
      ...
      super(Sequentialga, self).build(input_shape)
    self.built = True
  ...
  def call(self, inputs, training=None, mask=None):  # pylint: disable=redefined-outer-name
    ...
    if self._is_graph_network:
      if not self.built:
        self._init_graph_network(self.inputs, self.outputs, name=self.name)
      return super(Sequentialga, self).call(inputs, training=training, mask=mask)
  ...
  def get_config(self):
    ...
    # have the shape information to reconstruct `Sequentialga` as a graph network.
  ...
  def _trackable_saved_model_saver(self):
    return model_serialization.SequentialgaSavedModelSaver(self)
