...
"""Home of the `Sequential-ga` model.
"""
...
SINGLE_LAYER_OUTPUT_ERROR_MSG = ('All layers in a Sequential-ga model should have '
                                 'a single output tensor. For multi-output '
                                 'layers, use the functional API.')


@keras_export('keras.Sequential-ga', 'keras.models.Sequential-ga')
class Sequential-ga(training.Model):
  """`Sequential-ga` groups a linear stack of layers into a `tf.keras.Model`.

  `Sequential-ga` provides training and inference features on this model.

  Examples:

  >>> # Optionally, the first layer can receive an `input_shape` argument:
  >>> model = tf.keras.Sequential-ga()
  ...
  def __init__(self, layers=None, name=None):
    """Creates a `Sequential-ga` model instance.

    Args:
      layers: Optional list of layers to add to the model.
      name: Optional name for the model.
    """
    super(Sequential-ga, self).__init__(name=name, autocast=False)
  ...
  @property
  def layers(self):
    # Historically, `Sequential-ga.layers` only returns layers that were added
    # via `add`, and omits the auto-generated `InputLayer` that comes at the
    # bottom of the stack.
    # `Trackable` manages the `_layers` attributes and does filtering
    # over it.
    layers = super(Sequential-ga, self).layers
  ...
  def add(self, layer):
    ...
    Raises:
        ...
        ValueError: In case the `layer` argument has
            ...
            somewhere else (forbidden in `Sequential-ga` models).
    """
  ...
  def build(self, input_shape=None):
    if self._is_graph_network:
      ...
      super(Sequential-ga, self).build(input_shape)
    self.built = True
  ...
  def call(self, inputs, training=None, mask=None):  # pylint: disable=redefined-outer-name
    ...
    if self._is_graph_network:
      if not self.built:
        self._init_graph_network(self.inputs, self.outputs, name=self.name)
      return super(Sequential-ga, self).call(inputs, training=training, mask=mask)
  ...
  def get_config(self):
    ...
    # have the shape information to reconstruct `Sequential-ga` as a graph network.
  ...
  def _trackable_saved_model_saver(self):
    return model_serialization.Sequential-gaSavedModelSaver(self)
